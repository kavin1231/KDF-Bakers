...
db.on("error", (err) => {
  ...
  if (err.name === "MongoServerError" && err.codeName === "AtlasError") {
    console.error(
      "Authentication failed: Please check your MongoDB URI credentials in the .env file."
    );
  }
...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    const { customerName, email, phone, cakeSize, cakeFlavor, deliveryDate } =
      req.body;
    if (
      !customerName ||
      !email ||
...
app.post("/api/messages", async (req, res) => {
  try {
    ...
    if (!name || !email || !message) {
      return res
        .status(400)
        .json({ message: "Missing required fields for message." });
    }
    ...
    if (error.name === "ValidationError") {
      return res
        .status(400)
        .json({ message: "Message validation failed.", errors: error.errors });
    }
